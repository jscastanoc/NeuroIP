...
SNR = [-7];
NDIP = [3];
COORDS =[15 -20 15; 15 10 25; -15 -20 15; 15 5 25 ];
dir = '';
for ndip = NDIP
    [J, idx] = nip_simulate_activity(model.cortex, Laplacian,COORDS(1:ndip,:), ...
        act(1:ndip,:),model.t);
    J = fuzzy*J;
    J = sparse(J);
    y_clean = model.L*J;
    ...
    for snr = SNR
        ...
        for m_idx = 1:length(methods);
            ...
            fig(m_idx) = figure('Units','normalized','position',[0.2 0.2 0.14 0.14]);
            switch method
                case 'BMF'
            ...
            file_name = strcat(dir,'head',method,'_',num2str(ndip));
            savefig(file_name,gcf,'eps')
