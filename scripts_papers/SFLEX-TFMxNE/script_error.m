    ...
    for c_meth = 1:numel(methods)
        for j = 1:n_exp
            ...
            for l = 1:length(act_sources)
                for i = Ntrials
                    dir = strcat('/mnt/data/ResultadosBerlin/Results_final/',num2str(act_sources(l)));
                    ...
                    try
                    load(file_name)
                        J_rec = mgjob.results{find(ismember(mgjob.argout,{'J_rec'}))};
                        dir = strcat('/mnt/data/ResultadosBerlin/sim_trials_final/',num2str(act_sources(l)));
                        ...
                        switch c_er
                            case 1
                                ...
                                dir = strcat('/mnt/data/ResultadosBerlin/Error1/',num2str(act_sources(l)));
                            case 2
%                                 sig1 = sqrt(sum(J_rec.*J_clean,2));
%                                 sig2 = sqrt(sum(J_clean.*J_clean,2));
                                sig1 = sum(J_rec.*J_clean,2);
                                sig2 = sum(J_clean.*J_clean,2);
                                distance_c = norm(sig1-sig2,2);
                                dir = strcat('/mnt/data/ResultadosBerlin/Error2alt2/',num2str(act_sources(l)));
                        end
