...
% Structure with several useful fields with the data we've declared so far
...
%% Simulate Brain activity and pseudo-EEG
rng(1);
...
[J, ~] = nip_simulate_activity(model.cortex.vc, [30 -20 30;-30 20 30] , act,  randn(size(act,1),3), model.t, struct('sample_all',0));


% Simulate "smooth activity" (simulation is spatially low pass filtered)
...
% modified to get the desired resnorm. If false, then the user-select regularization parameters is used.
[J_eststout, extras] = nip_stout_python(model.y, L, B,'optimres',true,...
    'sreg',spatial_reg,'treg',temp_reg,'resnorm', resnorm, 'tstep',a ,'wsize',m,'Winv',Winv);
